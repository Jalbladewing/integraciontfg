...
public class Recursos_tecnico extends Recursos_tecnico_Ventana implements View
{
	@Autowired
	private InstalacionRepository instalacionRepo;
	
	public static String VIEW_NAME = "recursosTecnico";
		
	public Recursos_tecnico()
	{
		...
		guardarB.addClickListener(new Button.ClickListener() 
		{
			...
			{
				String value = Recursos_tecnico.updateInstalacion(instalacionMetal, instalacionOro, instalacionPetroleo, layoutMetal.tasaInstalacionF.getValue(), layoutOro.tasaInstalacionF.getValue(), layoutPetroleo.tasaInstalacionF.getValue(), instalacionRepo);
				errorL.setValue(value);
				
				if(value.isEmpty())
				{
					errorL.setVisible(false);
					doNavigate(Recursos_tecnico.VIEW_NAME + "/" + "creado");
				}else
				{
